...
lang$shortMonths <- c("Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez")
lang$weekdays <- c("Domingo","Segunda","Terça","Quarta","Quinta","Sexta","Sábado")
options(highcharter.lang = lang)
rm(lang)

# Opções gráficos
lista_opcoes_grafico <- c("downloadPNG", "downloadJPEG", "downloadPDF", "downloadSVG")

# Conexão com o banco de dados
conn <- dbConnect(SQLite(), "data/Indicadores.db")

# Indicadores
indi <- tbl(conn, "Indicadores") %>%
  collect() %>%
  # Patch para indicadores repetidos
  distinct() %>%
  mutate(
    valor = as.numeric(stringr::str_replace(string = valor, pattern = ",", replacement = ".")),
    interv_inf = as.numeric(stringr::str_replace(string = interv_inf, pattern = ",", replacement = ".")),
    interv_sup = as.numeric(stringr::str_replace(string = interv_sup, pattern = ",", replacement = ".")),
    cv = as.numeric(stringr::str_replace(string = cv, pattern = ",", replacement = "."))
  ) %>%
  mutate(
    abr_tipo = recode(
      abr_tipo,
    ...
    sidebarLayout(
      sidebarPanel(width = 12,
                   fluidRow(
                     column(
                       width = 12,
                       selectInput(inputId = "sel_modulo", label = "Módulo", choices = c(
                         "Q - Doenças Crônicas",
                   ...
                   fluidRow(
                     ...
                     column(
                       width = 4,
                       actionButton(inputId = "indi_def", label = "Ver definição do indicador", icon = icon("info"), class = "btn-primary"),
                       tags$style(type='text/css', "#indi_def { width:100%; margin-top: 25px;}")
                     )
                   ),
                   fluidRow(
                     column(
      ...
      mainPanel(width = 12,
                tabsetPanel(
                  type = "tabs",
                  tabPanel("Visualização", br(), uiOutput(outputId = "grafico_UI"), br()),
                  tabPanel("Tabela", br(), dataTableOutput("tabela"), br()),
                  tabPanel("Download", 
                           br(), 
                           downloadButton(outputId = "download_csv", label = "Formato CSV separado por ponto-e-virgula"), 
                           br(), br(),
    ...
    sidebarLayout(
      sidebarPanel(width = 12,
                   fluidRow(
                     column(
                       width = 12,
                       selectInput(inputId = "comp_sel_modulo", label = "Módulo", choices = c(
                         "Q - Doenças Crônicas",
                         "N - Percepção do Estado de Saúde",
                         "P - Estilos de vida / Alimentação",
                         "P - Estilos de vida / Consumo de álcool",
...
server <- function(input, output) {
  ...
  output$sel_indicador_UI <- renderUI({
    ...
    if(input$sel_modulo == "O - Acidentes"){
      ops <- c("Usa sempre cinto quando dirige automóvel", "Usa sempre cinto quando anda na frente como passageiro de automóvel", "Usa sempre cinto quando anda no banco de trás de automóvel", "Usa sempre capacete quando dirige motocicleta", "Usa sempre capacete quando está como passageiro de motocicleta", "Manuseio de celular durante a condução de veículo", "Acidente de trânsito com lesão corporal nos últimos 12 meses")
    } else {
      ops <- dic %>%
  ...
  output$sel_abr_tipo_UI <- renderUI({
    ...
    ops <- indi %>%
      ...
      select(abr_tipo) %>%
      distinct(abr_tipo) %>%
      arrange(factor(abr_tipo, levels = c("Total", "Grandes Regiões", "Unidades da Federação", "Capitais",
                                          "Situação urbano/rural", "Sexo", "Faixa de idade", "Faixa de idade (18 anos ou mais)", 
                                          "Raça/Cor", "Escolaridade", "Rendimento domiciliar per capita"))) %>%
      pull(abr_tipo)
  ...
  output$sel_grafico_UI <- renderUI({
    ...
    if(input$sel_abr_tipo == "Unidades da Federação" & input$sel_ano %in% c("2013", "2019")){
      ops <- c("Barras", "Mapa")
    } else {
      ops <- c("Barras")
    }
  ...
  output$sel_eixo_x_UI <- renderUI({
    radioButtons("sel_eixo_x", label = "Escala do gráfico",
  ...
  output$distPlot <- renderHighchart({
    ...
    titulo <- paste0(input$sel_indicador, " - ", input$sel_unidade, " - ", input$sel_ano)
    ...
    if(input$sel_abr_tipo == "Unidades da Federação"){
      ...
      cor_barra <- "orchid"
    } else if(input$sel_abr_tipo == "Escolaridade do responsável pelo domicílio"){
      cor_barra <- "violet"
      ...
      cor_barra <- "coral"
    } else if(input$sel_abr_tipo == "Escolaridade dos indivíduos de 18 anos ou mais"){
      cor_barra <- "cyan"
      ...
      if(nrow(dados()) > 0){
        ...
        if(input$sel_ano == "2013 - 2019"){
          ...
          indi_chart <- highchart() %>% 
            ...
            hc_exporting(
              enabled = TRUE,
              buttons = list(contextButton = list(menuItems = lista_opcoes_grafico))
            ) %>%
            hc_credits(enabled = TRUE, text = "Fiocruz | ICICT | LIS | PCDaS | IBGE",
                       href = "https://bigdata.icict.fiocruz.br"
            ) %>%
            hc_add_series(type = "bar", data = res, hcaes(y = valor, x = abr_nome, group = ano), color = c("purple","orange")) %>%
            hc_add_series(data = res, type = "errorbar", hcaes(x = abr_nome, low = interv_inf, high = interv_sup, group = label_error_bar, grouping = TRUE))
          ...
          if(input$sel_abr_tipo == "Unidades da Federação"){
            indi_chart <- indi_chart %>%
              hc_add_series(type = "bar", data = dados(), hcaes(y = valor, x = abr_nome, color = c(rep("#377EB8", 7), rep("#4DAF4A", 9), rep("#984EA3", 4), rep("#FF7F00", 3), rep("#00CCCC", 4))), name = "Valor")
          } else if(input$sel_abr_tipo == "Grandes Regiões"){
            indi_chart <- indi_chart %>%
              hc_add_series(type = "bar", data = dados(), hcaes(y = valor, x = abr_nome, color = c("#377EB8","#4DAF4A","#984EA3","#FF7F00","#00CCCC")), name = "Valor")
          } else {
            indi_chart <- indi_chart %>%
              hc_add_series(type = "bar", data = dados(), hcaes(y = valor, x = abr_nome), color = cor_barra, name = "Valor")
          }
          
          # Adiciona intervalo de confiança
          indi_chart <- indi_chart %>%
            hc_add_series(data = list_parse(mutate(dados(), low = interv_inf, high = interv_sup)),
                          type = "errorbar", color = "black", name = "Intervalo de confiança")
          ...
          if(length(lista_cv) > 0){
            indi_chart <- indi_chart %>%
              hc_caption(text = paste0("<b>CVs maiores que 30%.</b> (", paste(lista_cv, collapse = ", "), ")"))
          }
  ...
  output$distMap <- renderLeaflet({
    ...
    map <- leaflet() %>%
      ...
      addProviderTiles(provider = providers$CartoDB.Voyager, group = "Estradas") %>%
      addGlPolygons(data = res, group = "Indicador", color = ~pal(valor), popup = paste0("<b>", res$name_state, "</b>", "<br>", format(res$valor, decimal = ","), " (", format(res$interv_inf, decimal = ","), " - ", format(res$interv_sup, decimal = ","),  ")")) %>%
    ...
    if(input$sel_eixo_x == 1){
      map <- map %>%
        addLegend(title = NULL, pal = pal, values = c(0, 100), position = "bottomright", opacity = 80, labFormat = labelFormat(digits = 2, big.mark = ""))
    } else {
      map <- map %>%
        addLegend(title = NULL, pal = pal, values = res$valor, position = "bottomright", opacity = 80, labFormat = labelFormat(digits = 2, big.mark = ""))
    }
  ...
  output$tabela <- renderDataTable({
    ...
    datatable(res, options = list(
      ...
      formatRound(digits = 1, dec.mark = ",", columns = c("Valor", "Limite inferior", "Limite superior"))
  })
  ...
  output$comp_sel_abr_UI <- renderUI({
    ...
    res2 <- indi %>%
      ...
      distinct(abr_tipo) %>%
      filter(abr_tipo %in% c("Total", "Grandes Regiões", "Unidades da Federação", "Capitais")) %>%
      pull(abr_tipo) 
    
    res2 <- sort(factor(res2, levels = c("Total", "Grandes Regiões", "Unidades da Federação", "Capitais"), ordered = TRUE))
  ...
  output$comp_sel_abr_elemento_UI <- renderUI({
    ...
    selectInput(inputId = "comp_sel_abr_elemento", label = "Unidade", choices = res)
    
  })
  
  output$comp_sel_eixo_x_UI <- renderUI({
    radioButtons("comp_sel_eixo_x", label = "Escala do gráfico",
                 choices = list("Fixo em 100%" = 1, "Adaptativo" = 2), 
                 selected = 2)
  })
  ...
  output$comp_grafico <- renderHighchart({
    ...
    if(nrow(indi_grafico) > 0){
      if(length(unique(indi_grafico$ano)) == 2){
        titulo_grafico <- paste0(input$comp_sel_modulo, " - ", input$comp_sel_abr_elemento, " - 2013 - 2019")
        ...
        res <- highchart() %>% 
          ...
          hc_add_series(type = "bar", data = indi_grafico, hcaes(y = valor, x = nome, group = ano), color = c("purple","orange")) %>%
          hc_add_series(data = indi_grafico, type = "errorbar", hcaes(x = nome, low = interv_inf, high = interv_sup, group = label_error_bar, grouping = TRUE))
        ...
        titulo_grafico <- paste0(input$comp_sel_modulo, " - ", input$comp_sel_abr_elemento, " - ", unique(indi_grafico$ano)[1])
        ...
        res <- highchart() %>% 
          ...
          hc_add_series(type = "bar", data = indi_grafico, hcaes(y = valor, x = nome), color = ifelse(unique(indi_grafico$ano)[1] == "2019", "purple", "orange"), name = "Valor") %>%
          hc_add_series(data = list_parse(mutate(indi_grafico, low = interv_inf, high = interv_sup)),
                        type = "errorbar", color = "black", name = "Intervalo de confiança")
